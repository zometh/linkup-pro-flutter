...
class EditJobDialog extends ConsumerStatefulWidget {
  final JobModel job;

  const EditJobDialog({super.key, required this.job});

  @override
  ConsumerState<EditJobDialog> createState() => _EditJobDialogState();
}

class _EditJobDialogState extends ConsumerState<EditJobDialog> {
  ...
  void initState() {
    ...
    _isCurrent = widget.job.isCurrent;
    _selectedCompany = widget.job.company;
  }
  ...
  Future<void> _submit() async {
    ...
    final success = await ref
        ...
        .updateJob(widget.job.id, jobEntity);
    ...
    if (mounted) {
      ...
      if (success) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Expérience modifiée avec succès')),
        );
      } else {
        ScaffoldMessenger.of(context).showSnackBar(
